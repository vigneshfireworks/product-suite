...
export default function PartnerBusinessLayout({ children }: { children: React.ReactNode }) {
  ...
  const base = `/partner/businesses/${id}`;
  ...
  return (
    <div>
      ...
      {/* Sub-navigation tabs */}
      <div
        className="flex gap-1 mb-6 bg-white rounded-2xl p-1.5 shadow-card overflow-x-auto"
        style={{ border: "1px solid #f0f0f0", scrollbarWidth: "none" }}
      >
        {subNav.map(item => (
          <Link
            key={item.href}
